            ...
            foreach (var f in systemType.GetFields (BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)) {
                ...
                foreach (var pair in injections)
                {
                    if (f.FieldType.IsAssignableFrom(pair.Key))
                    {
                        ...
                        notFoundTypes.Remove(f.FieldType);
                        break;
                    }
                    
                    if(notFoundTypes.Contains(f.FieldType) == false)
                        notFoundTypes.Add(f.FieldType);
                }
            }

            if (notFoundTypes.Count > 0)
            {
                var exceptions = notFoundTypes.Select(type => new DependencyInstallationFailedException(systemType.ToString(), type.ToString())).ToList();
#if UNITY_EDITOR
                UnityEngine.Debug.LogError($"Failed to install dependencies. Not found {exceptions.Count} dependencies");
                foreach (var exception in exceptions) UnityEngine.Debug.LogException(exception);
#else
                throw exceptions[0];
#endif
            }
